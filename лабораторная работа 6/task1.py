...
def to_csv_file (filename, headers, rows, delimiter = ",", new_line = "\n"):
    with open(OUTPUT_FILE, "w") as filename:
        filename.write(delimiter.join(str(each_head) for each_head in headers) + new_line)
        for each_line in rows:
            filename.write(delimiter.join(str(each_row) for each_row in each_line) + new_line)
